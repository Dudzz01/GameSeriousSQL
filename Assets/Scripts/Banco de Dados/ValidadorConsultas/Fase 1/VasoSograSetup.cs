...
public class VasoSograSetup : MonoBehaviour
{
    // idMovel do Vaso da Sogra
    const int ExpectedMovelId = 2;
    ...
    void Awake()
    {
        ...
        fi.validator = new Func<object, bool>(itemsObj =>
        {
            ...
            foreach (var item in lista)
            {
                ...
                if (prop == null)
                {
                    Debug.Log("Validator: objeto IdMovel sem propriedade");
                    return false;
                }
            ...
            if (GameController.s.quantidadesDesafiosConcluidos[1] == false)
            {
                GameController.s.quantidadesDesafiosConcluidos[1] = true;
