...
public class PiaVizinhoSetup : MonoBehaviour
{
    const int ExpectedComodoId = 12;
    const int ExpectedMovelId = 47;
    ...
    void Awake()
    {
        ...
        fi.validator = new Func<string, object, bool>((sql, itemsObj) =>
        {
            ...
            if (nomeItemVal != null && nomeItemVal != "ChavePiaVizinho")
                return DebugFail($"NomeItem='{nomeItemVal}' (esperado 'ChavePiaVizinho' ou null)");
            ...
            if (GameController.s.quantidadesDesafiosConcluidos[25] == false)
            {
                GameController.s.quantidadesDesafiosConcluidos[25] = true;
