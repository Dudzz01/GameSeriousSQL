...
public class EstanteVizinhoSetup : MonoBehaviour
{
    const int ExpectedComodoId = 11;
    const int ExpectedMovelId = 43;
    ...
    void Awake()
    {
        ...
        fi.validator = new Func<string, object, bool>((sql, itemsObj) =>
        {
            ...
            if (nomeItemVal != null && nomeItemVal != "ChaveEstanteVizinho")
                return DebugFail($"NomeItem='{nomeItemVal}' (esperado 'ChaveEstanteVizinho' ou null)");
            ...
            if (GameController.s.quantidadesDesafiosConcluidos[29] == false)
            {
                GameController.s.quantidadesDesafiosConcluidos[29] = true;
