...
public class FogaoCozinhaAbandonado : MonoBehaviour
{

    const int ExpectedMovelId = 60;
    const int ExpectedQtdItens = 5;
    const int ExpectedQtdChaves = 1;
    ...
    void Awake()
    {
        ...
        fi.validator = new Func<string, object, bool>((sql, itemsObj) =>
        {
            ...
            if (GameController.s.quantidadesDesafiosConcluidos[30] == false)
            {
                GameController.s.quantidadesDesafiosConcluidos[30] = true;
