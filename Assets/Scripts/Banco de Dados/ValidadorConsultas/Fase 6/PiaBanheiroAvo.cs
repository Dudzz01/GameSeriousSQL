...
public class PiaBanheiroAvo : MonoBehaviour
{

    const int ExpectedMovelId = 76;  // o IdMovel do desafio
    const int ExpectedQtdItens = 5;   // quantos itens existem nesse móvel
    const int ExpectedQtdChaves = 1;   // quantas chaves existem (SUM NomeItem LIKE 'Chave%')
    ...
    void Awake()
    {
        ...
        fi.validator = new Func<string, object, bool>((sql, itemsObj) =>
        {
            ...
            if (GameController.s.quantidadesDesafiosConcluidos[35] == false)
            {
                GameController.s.quantidadesDesafiosConcluidos[35] = true;
